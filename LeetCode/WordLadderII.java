...
class Solution {
    ...
    private void findLadders(String current, String end, ArrayList<String> path, ArrayList<ArrayList<String>> ladders, HashSet<String> available, HashMap<String, ArrayList<String>> wordNexts, HashMap<String, Integer> cache, int min) {
        // skip longer path
        ...
        for(String str : wordNexts.get(current)) {
            ...
            if(available.contains(str)) {
                ...
                findLadders(str, end, path, ladders, available, wordNexts, cache, min);
                available.add(str);
            }
        }
    ...
    private int findMinLength(String start, String end, HashSet<String> dict, HashMap<String, ArrayList<String>> wordNexts) {
        ...
        while(!queue.isEmpty()) {
            length++;
            LinkedList<String> queueTemp = new LinkedList<String>();
            while(!queue.isEmpty()) {
                String str = queue.remove();
                if(cache.contains(str)) continue;
                else cache.add(str);
                if(str.equals(end)) return length;
                queueTemp.addAll(wordNexts.get(str));
            }
            queue = queueTemp;
        }
        return 0;
    ...
    public ArrayList<ArrayList<String>> findLadders(String start, String end, HashSet<String> dict) {
        ...
        // BFS to find minimum length
        int min = findMinLength(start, end, dict, wordNexts);
        ArrayList<ArrayList<String>> ladders = new ArrayList<ArrayList<String>>();
        // O(n^2)
        dict.remove(start);
        // DFS to find paths
        findLadders(start, end, new ArrayList<String>(), ladders, dict, wordNexts, new HashMap<String, Integer>(), min);
        return ladders;
    }
}
...
class Solution2 {
    private class NodeHelper {
        ...
        public NodeHelper(String word, NodeHelper prev) {
            this.word = word;
            this.prev = prev;
        }
    }
