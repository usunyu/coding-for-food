...
The new items are appended to the end and the order is retained i.e elements are placed in increasing order of 
their insertion time. When the number of elements in the list elements exceeds the defined size, the older 
...
class Solution {
	...
	public static void initialBuffer(int n) {
		Start = 0;
		Size = 0;
		Buffer = new String[n];
	}
	...
	public static void list() {
		...
		int num = 0;
		while(num < Size) {
			int index = (Start + num) % Buffer.length;
	...
	public static void main(String[] args) {
		try{
            ...
            while(true) {
            	...
            	if(input.charAt(0) == 'A') {	// A n - append the following n lines to the buffer.
            	...
            	else if(input.charAt(0) == 'R') {	// R n - remove first n elements of the buffer.
            		int num = Integer.parseInt(input.substring(2));
            		while(num > 0) {
            			remove();
            			num--;
            	...
            	else if(input.equals("L")) {	// L - list the buffer.
            		list();
            	}
            	else if(input.equals("Q")) {	// Q - Quit.
            		break;
            	}
            	else {
